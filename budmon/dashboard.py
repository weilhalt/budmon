...
class BudgetDashboard:
    ...
    def _do_refresh(self) -> None:
        ...
        if state.headers:
            self._last_limits = state.headers
        elif self._last_limits:
            # Re-parse from cached headers if current poll has none
            state = parse_quota_state({"headers_raw": self._last_limits})
